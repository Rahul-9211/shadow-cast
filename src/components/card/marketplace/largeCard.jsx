...
const LargeCard = ({ item }) => {
   return (
    <div className="group relative overflow-hidden rounded-lg bg-[#1D1D1D]">
      <ShadowFrame className="w-[200px] md:w-[300px] h-[200px] md:h-[300px] rounded-[200px] md:rounded-[500px] -left-[20px] md:-left-[100px] bottom-0 md:block hidden" />
      <div className="p-3">
        ...
        <div className="flex flex-wrap justify-between relative z-10 gap-5 items-center p-4 my-4">
          <div className="max-w-2xl xl:max-w-3xl">
            <h4 className="font-bold text-base md:text-xl mb-3 md:mb-5">
              {item?.title}
            </h4>
            <p className="leading-sung text-sm md:text-base text-white/[0.90]">
              {item?.desc}
            </p>
          </div>
          {item?.slug && (
            <NavLink
              to={item?.slug || null}
              className="inline-flex border text-sm md:text-base leading-none rounded-2xl border-sec btn-outline hover:text-black px-6 py-3 md:py-4 md:px-8 font-medium"
            >
              See Apartment
            </NavLink>
          )}
