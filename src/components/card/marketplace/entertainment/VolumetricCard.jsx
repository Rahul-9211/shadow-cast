...
const VolumetricCard = ({ slide, title, subtitleButton , tagType , tags ,thumbnailIcon }) => {
  return (
    <div className="mb-12">
      <div className="aspect-[456/239] relative rounded-2xl bg-[#2B2B2B] mb-2.5 ">
      {tags && <span className='tags'>{tags}</span>}
      {thumbnailIcon  && <span className='music-icon'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      {tagType === "Free" && <button className="tag-item px-1">Free</button>}
      {tagType === "Paid"  && (
        <button className="tag-item px-1">{subtitleButton}</button>
      )}
      {tagType === "Premium" && (
        <button className="tag-item flex gap-4 items-center px-1 ">
          <svg
