...
/*It defines a
component called `Index` that renders a list of 180 Video cards. */
const Index = () => {
    ...
    <>
      <Grid container item xs={12} className="fs24px justify-center">
        <Grid container item xs={11} sm={12} className="gap-16">
          <Grid item xs={12}>
            <Typography className="fs20px font-bold">180 Video</Typography>
          </Grid>
          ...
          {list?.map((item, i) => (
            <Card
              data={item}
              key={i}
              redirect={'/marketplace/entertainments/180'}
            />
          ))}
        </Grid>
      </Grid>
