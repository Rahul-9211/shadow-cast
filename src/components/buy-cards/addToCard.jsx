...
const AddToCard = ({ title, price, type, handleData }) => {
    const handleClick = () => {
        handleData('BuyNow');
    }
    return (
        <>
            ...
            <button type="button" className="font-bold text-sm md:text-base rounded-lg btn-gradient hover:text-white w-full text-black py-3 px-5 mt-6" onClick={handleClick}>{type==="content" ? 'Buy @'+price : 'Buy Now'}</button>
